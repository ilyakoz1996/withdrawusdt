...
export default function Transactions() {
  ...
  return (
    <div className="flex flex-col">
      ...
      <button disabled={dataLoading} onClick={getLastData} className={`py-1 px-2 font-bold hover:text-white rounded-md border hover:bg-blue-500 ${dataLoading && 'bg-blue-400 text-white'}`}>
        {dataLoading ? <div className="flex space-x-2 items-center">
      ...
      <table className="table-auto">
        ...
        <tbody>
          ...
          transactions.map((tx) => {
            return (
              <tr key={tx.time} className="odd:bg-white even:bg-gray-100 hover:bg-gray-200 cursor-pointer">
                <td className="text-center px-2">{tx.from ? shortAddress(tx.from) : 'unknown'}</td>
                <td className="text-center px-2">{tx.amount}</td>
                <td className="text-center px-2 b"><span className={`px-2 bg-${tx.event === 'Provide' ? 'blue-300' : 'green-200'} rounded-md`}>{tx.event}</span></td>
